...
const FundCard = ({ owner,name, title, description, target, duedate, amountCollected, image, handleClick }) => {
  const remainingDays = daysLeft(duedate);
    ...
    <div className="sm:w-[288px] w-full rounded-[15px] bg-[#1c1c24] cursor-pointer" onClick={handleClick}>
      ...
      <div className="flex flex-col p-4">
        <div className="flex flex-row items-center mb-[18px]">
          ...
          <p className="ml-[12px] mt-[2px] font-epilogue font-medium text-[12px] text-[#808191]">Infaq</p>
        </div>
        ...
        <div className="flex items-center mt-[20px] gap-[12px]">
          ...
          <p className="flex-1 font-epilogue font-normal text-[12px] text-[#808191] truncate">by <span className="text-[#b2b3bd]">{title}</span></p>
        </div>
      </div>
    </div>
  )
